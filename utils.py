...
def coo2dense(coo_mat):
    # if torch.__version__ <= 1.8.0:
    indices = coo_mat.coalesce().indices()
    values = coo_mat.coalesce().values()
    #print(coo_mat.size())
    dense_mat = torch.zeros((coo_mat.size()))
    for i in range(len(values)):
        dense_mat[indices[0,i],indices[1,i]] = values[i]
    # print(dense_mat)
    # assert 0
    return dense_mat
...
class PlotUtils(object):
    ...
    def plot_subgraph(
        ...
        nodelist = [int(node) for node in nodelist]
        ...
        nx.draw_networkx_nodes(graph, pos,
                               nodelist=nodelist, node_color=subgraph_edge_color,
                               node_size=400)

        if labels is not None:
            nx.draw_networkx_labels(graph, pos, {i:str(i) for i in list(graph.nodes())})
    ...
    def plot_ba2motifs(
        ...
        return self.plot_subgraph(
            ...
            title_sentence=title_sentence,
            figname=figname,
            labels = {i:str(i) for i in list(graph.nodes())}
        )
