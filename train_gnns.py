...
def main(config):
    ...
    count_node = 0
    for data in dataset:
        count += data.edge_index.shape[1]
        count_node += data.x.shape[0]
    print(count, count_node)
    assert 0
    ...
    test_loss, test_acc, preds = trainer.test()
    
    # js_info = {
    #     'dataset_name' : config.datasets.dataset_name,
    #     'hidden' : trainer.model.gnn_latent_dim,
    #     'pool' : config.models.param.readout,
    #     'test_acc' : test_acc 
        
    # }
    # with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'pre_trained_gnn_accuracy.json'), 'a+') as f:
    #     json.dump(js_info, f, indent=4)
