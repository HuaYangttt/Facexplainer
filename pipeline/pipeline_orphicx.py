...
def pipeline(config):
    ...
    if os.path.isfile(orphicx_data_saving_path):
        ...
        if config.datasets.dataset_name != 'redditbinary':
    ...
    if not IS_FRESH and os.path.isfile(orphicx_saving_path):
        ...
        logger.info("trainging orphicx model...")
        best_loss = 100
    ...
    related_preds_list = []
    logger.info("Evaluating orphicx model...")
    ...
    metrics_str += f'{cost_time: .4f}, {len(test_indices)}, {train_using_time: .4f}'
