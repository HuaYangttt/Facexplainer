...
def pipeline(config):
    ...
    # assert 0, config.models.param.graph_classification
    ...
    for i, data in enumerate(tqdm(dataset[test_indices])):
        ...
        # print(config.datasets.dataset_name, idx, time.time() - s_time)
        ...
        if config.save_plot:
            ...
            from methods.subgraphx import find_closest_node_result
